<footer class="do-footer">
    <div class="container">
        <div class="row">

            <!-- FOOTER TOP -->
            <div class="do-footer-top">
                <!-- About Widget -->
                <div class="do-footer-widget do-footer-about-widget col-md-3 col-sm-6 col-xs-12">
                    <div class="do-footer-logo-wrapper">
                        <a href="index.html#">
                            <img src="images/Logo_338_white.png" alt="">
                        </a>
                    </div>

                    <div class="do-company-address">
                        <ul>
                            <li>
                                <i class="fa fa-map-marker"></i> #323, Defence Avenue ,<br/> K Narayanpura, Bengaluru - 77
                            </li>
                            <li>
                                <i class="fa fa-phone"></i> +91-890-433-0374
                                <br/>+91-805-061-1198
                            </li>
                            <li>
                                <i class="fa fa-envelope-o"></i> hello@three38inc.com
                            </li>
                        </ul>
                    </div>
                </div>
                <!-- About Widget End -->

                <!-- Link Widget -->
                <!--
                    <div class="do-footer-widget do-footer-link-widget col-md-3 col-sm-6 col-xs-12">
                        <h3 class="do-footer-widget-header">USEFUL LINKS</h3>
                        <ul>
                            <li><a href="index.php">Home</a></li>
                            <li><a href="about-us.php">About Us</a></li>
                            <li><a href="services.php">services</a></li>
                            <li><a href="portfolio.php">Portfolio</a></li>
                            <li><a href="contact-us.php">Contact Us</a></li>
                        </ul>
                    </div>
-->
                <!-- Link Widget End -->

                <!-- About Link Widget -->
                <div class="do-footer-widget do-about-link-widget col-md-offset-3 col-md-3 col-sm-6 col-xs-12">
                    <h3 class="do-footer-widget-header">USEFUL LINKS</h3>
                    <ul>
                        <li><a href="index.php">Home</a></li>
                        <li><a href="index.php#about-us">About Us</a></li>
                        <li><a href="index.php#services">services</a></li>
                        <li><a href="index.php#portfolio">Portfolio</a></li>
                        <li><a href="contact-us.php">Contact Us</a></li>
                    </ul>
                </div>
                <!-- About Link Widget End -->
                <div class="do-footer-widget do-reach-link-widget  col-md-3 col-sm-6 col-xs-12">
                    <h3 class="do-footer-widget-header">Reach To Us</h3>
                    <form class="do-reach-form" id="do-reach-form" action="includes/reach-us.php">
                        <input type="text" name="name" id="name" placeholder="Name">
                        <input type="text" name="email" id="email" placeholder="Email">
                        <input type="text" name="contact" id="contact" placeholder="Contact">
                        <textarea name="message" id="message" cols="30" rows="10" placeholder="Comment" style="height: 70px;font-size:small;padding:10px;"></textarea>
                        <input type="submit" value="SEND" class="reach-submit do-btn-round-solid">
                    </form>

                </div>
                <div class="do-footer-widget do-reach-link-alert-success-widget  col-md-3 col-sm-6 col-xs-12">
                    <h4>Your Query has been submitted successfully.You would be contacted soon.</h4>
                    <span><i class="ti-thumb-up"></i></span>
                </div>
                <div class="do-footer-widget do-reach-link-alert-error-widget  col-md-3 col-sm-6 col-xs-12">
                    <h4>Error in your input format, Please provide valid input.</h4>
                    <span><i class="ti-thumb-down"></i></span>
                </div>
            </div>
            <!-- FOOTER TOP END -->

            <!-- FOOTER BOTTOM -->
            <div class="do-footer-bottom">
                <p class="do-footer-author-name">Copyrights ©
                    <?php echo date('Y')?> All Rights Reserved </p>
            </div>
            <!-- FOOTER BOTTOM END -->

        </div>

    </div>
</footer>
